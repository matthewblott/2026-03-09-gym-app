...
## Prefer HTML over Rails tag helpers
...
## Avoid form helpers when possible
...
## Avoid `content_tag` and similar helpers
...
## Avoid HTML generated entirely by Ruby
...
## Use Rails helpers only when necessary
...
## Use path helpers, never hardcoded URLs

All links and form actions must use Rails path helpers.
...
```erb
<a href="<%= todos_path %>">Cancel</a>
<form action="<%= todos_path %>" method="post">
```
